...
def dataloader(filepath):
 ...
 left_train_dir = os.path.join(filepath, 'clean_left_flatten_training/')
 right_train_dir = os.path.join(filepath, 'clean_right_flatten/')
 disp_train_dir = os.path.join(filepath, 'depth_flatten/')
 left_test_dir = os.path.join(filepath, 'clean_left_flatten_testing/')
 right_test_dir = os.path.join(filepath,  'clean_right_flatten/')
 disp_test_dir = os.path.join(filepath,  'depth_flatten/')


 # all_left_img = os.listdir(left_train_dir)
 file_list = os.listdir(left_train_dir)
 depth_list = [a.split('.')[0]+'.dpt' for a in file_list]
 all_left_img  = [left_train_dir  + s for s in file_list]
 all_right_img = [right_train_dir + s for s in file_list]
 all_left_disp = [disp_train_dir  + s for s in depth_list]
 ...
 test_left_disp = [disp_test_dir  + s for s in depth_list]
